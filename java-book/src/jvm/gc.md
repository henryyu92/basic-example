...
可达性分析算法需要从 `GC Roots` 集合开始通过引用链的方式找到存活对象，`GC Roots` 节点主要在全局性的引用(常量和静态变量)和执行上下文中，在实际中由于可以作为 GC Root 的节点非常的多，如果逐个从这些节点开始搜索则会消耗大量的时间。
...
漏标的情况会使被引用的对象被清除，影响程序的正确性。通过破坏漏标发生条件中的任意一个即可避免漏标的产生，因此解决漏标问题有两种方案：
