...
Go 程序是由多个小的基础构建组成的，包括表达式、数据结构、控制语句、函数等。Go 程序是以源码和包的方式被组织。

#### 命名
Go 语言中的所有命名都必须以字母或下划线开头，后面可以跟任意数量的字母和下划线，其中字母是大小写敏感的。Go 内置了 25 个关键字，不能使用关键字作为命名
```
...
Go 使用包来支持模块化和代码复用，一个包通常包含多个以 `.go` 结尾的源文件。包的导入路径通常是包所在的目录路径的后缀，对应一个命名空间，不同命名空间的变量、数据类型以及函数等即使有相同的命名也是不同的。

包由全局唯一的导入路径定位，每个 Go 语言源文件的开头都必须有包声明语句，包声明语句用于确定当前包被其他包导入时的默认包名。默认的包名是包导入路径名的最后一段，因此存在不同导入路径的包有相同的包名。
...
```
其中 `initialization` 是初始值表达式；`condition` 是执行条件，只有满足条件才能执行循环体；`post` 是在循环体执行结束后执行。`initialization` 省略则表示初始值在 for 语句之前存在；`post` 省略则表示循环体执行后不需要后续处理；`condition` 省略则表示 for 循环是无限循环。
```go
...
结构体可以作为函数的参数和返回值，需要注意函数的参数和返回值都是值拷贝(深度值拷贝)，也就是会拷贝结构体中的所有成员，且函数内对结构体的改动不会影响原始变量。如果要在函数内部修改结构体成员的话，用指针传入是必须的。
```go
...
```
go 编译器为方法调用提供了隐式的类型转换，也就是无论方法的接收器参数是指针类型还是非指针类型，都是可以通过指针/非指针类型进行调用：
```go
r := Point{1, 2}
