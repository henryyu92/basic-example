...
### 压缩列表
压缩列表(ziplist)是列表键和哈希键的底层实现。压缩列表是 Redis 为了节约内存而开发的，是由一些列特殊编码的连续内存块组成的顺序型(sequential)数据结构，一个压缩列表可以包含任意多个节点(entry)，每个节点可以保存一个字节数组或者一个整数值。

zlbytes|zltail|zllen|entry|entry|...|zlend
-|-|-|-|-|-|-

压缩列表是一块连续内存，其各个组成部分为：
- zlbytes 是 uint32_t 的整数，记录整个压缩列表占用的内存字节数，在对压缩列表进行内存重分配或者计算 zlend 时使用
- zltail 是 uint32_t 的整数，记录 ziplist 中最后一个 entry 距离压缩列表的起始地址有多少字节，从而无需遍历整个压缩列表可以确定表尾节点的地址，从而使得 push 和 pop 操作的时间复杂度为 O(1)
- zllen 是 uint16_t 的整数，记录了压缩列表包含的节点(entry)数量。当小于 UINT16_MAX 时表示压缩列表的节点数量，当大于 UINT16_MAX 时需要遍历压缩列表确定节点数量
- entry 表示压缩列表的节点，节点的长度由节点保存的内容决定
- zlend 是 uint8_t 整数，用于标记压缩列表的末端，固定为 0xFF
